...
public class PaymentApiClient {
    ...
    public PaymentApiClient(
            @Value("${payment.server.hostname:localhost}") String host,
            @Value("${payment.server.port:8081}") int port,
    ...
    public List<Responses.Bank> getBanks() {
        ...
        ResponseEntity<List<Responses.Bank>> res = restTemplate.exchange("http://%s:%d/api/providers/top?country=RO&count=20".formatted(host, port),
    ...
    public Responses.PaymentResponse createPayment(String email, Payment payment) {
        ...
        Requests.PaymentCreate requestBody = Requests.PaymentCreate.of(payment.getTargetAmount().targetAmount(),
                "SplitMate payment #" + payment.getId().id(),
        ...
        ResponseEntity<Responses.PaymentResponse> res = restTemplate.exchange("http://%s:%d/api/payments".formatted(host, port),
                HttpMethod.PUT, requestEntity, Responses.PaymentResponse.class);
    ...
    public Responses.PaymentResponse getPayment(Payment payment) {
        ...
        ResponseEntity<Responses.PaymentResponse> res = restTemplate.exchange("http://%s:%d/api/payments/{id}".formatted(host, port),
